...
float ShadowCalculation(vec3 FragPos,float angle)
{
    ...
    vec4 fragPosLightSpace = d_light.spaceMatrix[0]*vec4(FragPos,1.f);
    ...
    float closestDepth = texture(d_light.shadow_map[0], projCoords.xy).r; 
    ...
    // PCF
    float shadow = 0.0;
    vec2 texelSize = 1.0 / textureSize(d_light.shadow_map[0], 0);
    for(int x = -1; x <= 1; ++x)
    {
        for(int y = -1; y <= 1; ++y)
        {
            float pcfDepth = texture(d_light.shadow_map[0], projCoords.xy + vec2(x, y) * texelSize).r; 
