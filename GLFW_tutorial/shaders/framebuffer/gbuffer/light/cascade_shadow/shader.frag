...
float factorCascadeShadow(vec4 FragPos,float angle){
    ...
    for(int x = -halfkernelWidth; x <= halfkernelWidth; ++x)
    {
	    ...
	    {
		    float pcfDepth = texture(d_light.shadow_map, vec3(projCoords.xy + vec2(x, y) * texelSize, cascade_level)).r;
...
{     
    ...
    //DirectionLight-----------------------------------
        float dotNormal_Dir = dot(normalize(Normal), d_light.direction);
        //Направленный свет
    ...
    {    
        ...
        dotNormal_Dir = dot(Normal, lightDir);
