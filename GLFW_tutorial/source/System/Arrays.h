...
class Array2d {
public:
	inline T& operator ()(size_t x, size_t y) { return buffer[toInt(x, y, size_)]; }
	inline T& operator ()(const glm::uvec2& coord) { return buffer[toInt(coord, size_)]; }
	inline T& operator[](size_t index) { return buffer[index]; }
...
public:
	...
	inline T& operator ()(size_t x, size_t y, size_t z) { return buffer[ind(x, y, z)]; }
	inline const T& operator ()(size_t x, size_t y, size_t z)const { return buffer[ind(x, y, z)]; }
	inline T& operator ()(const glm::uvec3& coord) { return buffer[ind(coord)]; }
	inline const T& operator ()(const glm::uvec3& coord) const { return buffer[ind(coord)]; }
	inline T& operator[](size_t index) { return buffer[index]; }
	...
	// Проверка корректности адреса
	inline bool is(size_t x, size_t y, size_t z)const { return x < x_&& y < y_&& z < z_; }
	inline bool is(const glm::uvec3& coord)const { return coord.x < x_&& coord.y < y_&& coord.z < z_; }
	inline void setNull() { for (size_t i = 0; i < size(); i++) buffer[i] = 0; }

	// Если адрес неверный то возвращает 0
	inline  T* get(const glm::uvec3& coord) { return is(coord) ? &(buffer[ind(coord)]) : 0; }
	inline const T* get(const glm::uvec3& coord) const { return is(coord) ? &(buffer[ind(coord)]) : 0; }

	//Обрезает адрес
	inline const T& clip(const glm::uvec3& coord) const { return buffer[ind(::clip(coord.x, x_), ::clip(coord.y, y_), ::clip(coord.z, z_))]; }
	inline const T& clip(size_t x, size_t y, size_t z) const { return buffer[ind(::clip(x, x_), ::clip(y, y_), ::clip(z, z_))]; }
